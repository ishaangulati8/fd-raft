...
const (
	...
	snapshoterAppliedMsgInterval = 500
)
...
func (kv *KVServer) snapshoter(persister *Persister, snapshotTrigger <-chan bool) {
	...
	for !kv.killed() {
		...
		case <-time.After(time.Duration((1-ratio)*snapshoterCheckInterval) * time.Second):
		// waiting for trigger
